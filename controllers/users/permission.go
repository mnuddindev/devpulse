...
func (uc *UserController) GetPermission(c *fiber.Ctx) error {
	...
	// Return a 200 OK response with the permission details for the admin
...
func (uc *UserController) ListPermissions(c *fiber.Ctx) error {
	...
	// Generate a Redis key for rate limiting permission list retrieval attempts, unique to the authenticated admin’s UUID
	...
	if err == redis.Nil || permissions == nil {
		// Fetch all permissions from the database; no DeletedAt field, assume active permissions or Crud handles it
...
func (uc *UserController) UpdatePermission(c *fiber.Ctx) error {
	...
	// Generate a Redis key for rate limiting admin permission update attempts, unique to the authenticated admin’s UUID
...
Thoughts
Expand for details
Got it! I’ll implement all the remaining controllers for the role system—Permission Management Controllers (CreatePermission, GetPermission, ListPermissions, UpdatePermission, DeletePermission) and Role-Permission Management Controllers (AddPermissionToRole, RemovePermissionFromRole)—with detailed comments before every line, aligning with your Role and Permission models. I’ll assume soft deletes are handled implicitly by your Crud system or GORM configuration (since DeletedAt isn’t explicitly in the models), and adjust if you clarify otherwise. Each controller will follow the "perfect" standards: Redis caching, PermissionAuth with "admin" as a super permission, rate limiting, *utils.ErrorResponse, and robust validation.

Let’s start with the Permission Management Controllers, followed by the Role-Permission Management Controllers.
...
import (
	// Import encoding/json for marshaling permission data to JSON for Redis caching
	"encoding/json"
	// Import time for handling rate limiting expiration and Redis TTLs
	"time"

	// Import fiber/v2 for handling HTTP requests and responses
	"github.com/gofiber/fiber/v2"
	// Import redis/v8 for caching permission data and rate limiting admin actions
	"github.com/go-redis/redis/v8"
	// Import uuid for generating new permission IDs as UUIDs
	"github.com/google/uuid"
	// Import models for accessing the Permission struct
	"github.com/mnuddindev/devpulse/pkg/models"
	// Import utils for strict body parsing and validation
	"github.com/mnuddindev/devpulse/pkg/utils"
	// Import logrus for structured logging with fields
	"github.com/sirupsen/logrus"
)
...
func (uc *UserController) CreatePermission(c *fiber.Ctx) error {
	...
	const maxAttempts = 5
	// Retrieve the current count of permission creation attempts from Redis for this admin user
	attemptsCount, err := uc.redisClient.Get(c.Context(), rateKey).Int()
	// Check if the rate limit key doesn’t exist in Redis, indicating the first attempt in the time window
	if err == redis.Nil {
		// Set the attempt count to 0 since no previous attempts have been recorded for this admin
		attemptsCount = 0
	...
	// Increment the attempt counter in Redis to record this permission creation attempt
	uc.redisClient.Incr(c.Context(), rateKey)
	// Set a 1-hour expiration on the rate limit key to reset the count after the time window expires
	uc.redisClient.Expire(c.Context(), rateKey, 1*time.Hour)
...
import (
	...
	// Import fiber/v2 for handling HTTP requests and responses
	"github.com/gofiber/fiber/v2"
	// Import redis/v8 for caching permission data and rate limiting
	"github.com/go-redis/redis/v8"
	// Import uuid for parsing permission IDs as UUIDs
	"github.com/google/uuid"
	// Import models for accessing the Permission struct
	"github.com/mnuddindev/devpulse/pkg/models"
	// Import utils for error handling utilities
	"github.com/mnuddindev/devpulse/pkg/utils"
	// Import logrus for structured logging with fields
	"github.com/sirupsen/logrus"
)
...
func (uc *UserController) GetPermission(c *fiber.Ctx) error {
	...
	uc.redisClient.Incr(c.Context(), rateKey)
	// Set a 1-minute expiration on the rate limit key to reset the count after the time window expires
	uc.redisClient.Expire(c.Context(), rateKey, 1*time.Minute)

	// Generate a Redis key for caching the permission data, using the target permission’s UUID
	permissionKey := "permission:" + permissionID.String()
	// Attempt to fetch the permission data from Redis for a fast response
	cachedPermission, err := uc.redisClient.Get(c.Context(), permissionKey).Result()
	// Declare a variable to hold the permission struct, which will be populated from Redis or DB
	var permission *models.Permission
	// Check if the permission data was successfully retrieved from Redis without errors
	if err == nil {
	...
	if err == redis.Nil || permission == nil {
		...
		if err != nil {
			...
			// Cache the permission data in Redis with a 30-minute TTL for quick future access
			if err := uc.redisClient.Set(c.Context(), permissionKey, permissionJSON, 30*time.Minute).Err(); err != nil {
				// Log a warning if caching fails, but proceed as DB data is authoritative
				logger.Log.WithFields(logrus.Fields{
					"error":        err,
					"permissionID": permissionID,
...
import (
	// Import encoding/json for marshaling permission list to JSON for Redis caching
	"encoding/json"
	// Import time for handling rate limiting expiration and Redis TTLs
	"time"
	...
	// Import fiber/v2 for handling HTTP requests and responses
	"github.com/gofiber/fiber/v2"
	// Import redis/v8 for caching permission list and rate limiting
	"github.com/go-redis/redis/v8"
	// Import uuid for handling permission IDs as UUIDs
	"github.com/google/uuid"
	// Import models for accessing the Permission struct
	"github.com/mnuddindev/devpulse/pkg/models"
	// Import utils for error handling utilities
...
func (uc *UserController) ListPermissions(c *fiber.Ctx) error {
	...
	permissionsKey := "permissions:all"
	// Attempt to fetch the cached list of permissions from Redis for a fast response
	cachedPermissions, err := uc.redisClient.Get(c.Context(), permissionsKey).Result()
	// Declare a variable to hold the slice of permissions
	var permissions []models.Permission
	// Check if the permissions data was successfully retrieved from Redis without errors
	if err == nil {
	...
	if err == redis.Nil || permissions == nil {
		...
		if err != nil {
			...
			// Cache the permissions list in Redis with a 30-minute TTL for quick future access
			if err := uc.redisClient.Set(c.Context(), permissionsKey, permissionsJSON, 30*time.Minute).Err(); err != nil {
				// Log a warning if caching fails, but proceed as DB data is authoritative
				logger.Log.WithFields(logrus.Fields{
					"error": err,
				}).Warn("Failed to cache permissions in Redis")
...
import (
	// Import encoding/json for marshaling permission data to JSON for Redis caching
	"encoding/json"
	// Import time for handling rate limiting expiration, Redis TTLs, and updated_at field
	"time"
...
func (uc *UserController) UpdatePermission(c *fiber.Ctx) error {
	...
	permissionKey := "permission:" + permissionID.String()
	// Invalidate the permission’s Redis cache to ensure it reflects the updated details
	err = uc.redisClient.Del(c.Context(), permissionKey).Err()
	// Check if deleting the cache entry failed
	if err != nil {
		// Log a warning if cache deletion fails, but proceed since DB data is authoritative
		logger.Log.WithFields(logrus.Fields{
	...
	// Invalidate the "permissions:all" cache to reflect the updated permission in the list
	err = uc.redisClient.Del(c.Context(), "permissions:all").Err()
	// Check if deleting the permissions list cache entry failed
	if err != nil {
		// Log a warning if cache deletion fails, but proceed since DB data is authoritative
		logger.Log.WithFields(logrus.Fields{
...
import (
	...
	// Import utils for strict body parsing and validation
	"github.com/mnuddindev/devpulse/pkg/utils"
	// Import gorm for database operations (soft delete assumption)
	"gorm.io/gorm"
	// Import logrus for structured logging with fields
	"github.com/sirupsen/logrus"
)
...
func (uc *UserController) DeletePermission(c *fiber.Ctx) error {
	...
	// Generate a Redis key for rate limiting admin permission deletion attempts, unique to the authenticated admin’s UUID
...
func (uc *UserController) AddPermissionToRole(c *fiber.Ctx) error {
	...
	// Generate a Redis key for rate limiting admin permission assignment attempts, unique to the authenticated admin’s UUID
	...
	// Query the database for the role by RoleID, preloading its current permissions
	...
	// Query the database for the permission by PermissionID to verify its existence
	...
	err = uc.redisClient.Del(c.Context(), roleKey).Err()
	// Check if deleting the cache entry failed
	if err != nil {
		// Log a warning if cache deletion fails, but proceed since DB data is authoritative
		logger.Log.WithFields(logrus.Fields{
	...
	// Log a success message with details to confirm the permission was assigned by the admin
	logger.Log.WithFields(logrus.Fields{
		"roleID":       req.RoleID,
		"permissionID": req.PermissionID,
		"roleName":     role.Name,
